...
namespace GeneralUtilities
{
	public class XmlTableManager : IDisposable
	{
		...
		public bool Update(int id, string name, string value)
		{
			...
			if (doc == null) Load();
			XElement row = doc.Descendants(ROW).Single(r => r.Attribute(ID).Value == id.ToString());
			if (row == null)
			{
		...
		{
			if (doc == null) Load();
			XElement row = doc.Descendants(TABLE).Single(r => r.Attribute(ID).Value == id.ToString());
		...
		{
			if (doc == null) Load();
			doc.Descendants(TABLE).Single(r => r.Attribute(ID).Value == id.ToString()).Remove();
		}
		public int GetLastId()
		{
			if (doc == null) Load();
			var table = doc.Element(TABLE);
			// var row = table.Elements(ROW).OrderBy(d => (int)d.Attribute(ID)).Last();
